...
class CampTix_Payment_Method_UgMart extends CampTix_Payment_Method {

	public $id = 'paystack';
	public $name = 'Mobile Money Collections for Uganda';
	public $description;
	public $supported_currencies = array( 'UGX' );
	protected $options = array();

    function __construct() {
        $this->description = $this->ugmart_gateway_description();
		parent::__construct();
    }

    function ugmart_gateway_description() {
    	global $camptix;
		$notify_url = add_query_arg( array(
			'tix_action' => 'payment_notify',
			'tix_payment_method' => 'ugmart',
		), $camptix->get_tickets_url() );

		$description = '<strong>To open a holding Account: Go to <a href="https://app.ugmart.ug/register" target=_blank rel="noopener noreferrer">this URL</a></strong>';
		return $description;
    }
	...
	function payment_settings_fields() {
		...
		$this->add_settings_field_helper( 'password', 'Password', array( $this, 'field_text' ) );
		$this->add_settings_field_helper( 'collection_account', 'Collection Account', array( $this, 'field_text' ) );
	...
	function validate_options( $input ) {
		...
		if ( isset( $input['collection_account'] ) ) {
			$output['collection_account'] = $input['collection_account'];
		}
	...
	function payment_checkout( $payment_token ) {
		...
		if ( ! ( $public_key && $secret_key ) ) {
			wp_die( 'Kindly enter your Login keys on the CampTix payment page.' );
		}
		...
		$ugmart_url 	= 'https://api.paystack.co/transaction/initialize';
		$headers = array(
		...
		$request = wp_remote_post( $ugmart_url, $args );
        if ( ! is_wp_error( $request ) && 200 == wp_remote_retrieve_response_code( $request ) ) {
        	$ugmart_response = json_decode( wp_remote_retrieve_body( $request ) );
        	$transaction_reference = $ugmart_response->data->reference;
        	$payment_url = $ugmart_response->data->authorization_url;
        	update_post_meta( $order['attendee_id'], 'tix_transaction_id', $transaction_reference );
			wp_redirect( $payment_url );
			die();
        } else {
        	$ugmart_response = json_decode( wp_remote_retrieve_body( $request ) );
			$camptix->error( 'Paystack error: ' . $ugmart_response->message );
	...
	function payment_return() {
		...
		$options = $this->options;
		$ugmart_url 	= 'https://api.paystack.co/transaction/verify/'  . rawurlencode($reference);
		$headers = array(
			'Content-Type'	=> 'application/json',
			'Authorization' => 'Bearer ' . $secret_key,
			'Cache-Control'	=> 'no-cache'
		...
		$request = wp_remote_get( $ugmart_url, $args );
        if ( ! is_wp_error( $request ) && 200 == wp_remote_retrieve_response_code( $request ) ) {
	...
	function payment_notify() {
		global $camptix;
		if ( ( strtoupper( $_SERVER['REQUEST_METHOD'] ) != 'POST' ) || ! array_key_exists('HTTP_X_ugmart_SIGNATURE', $_SERVER) ) {
			exit;
		}
	    $json = file_get_contents( "php://input" );
	    $options = $this->options;
		// validate event do all at once to avoid timing attack
		if ( $_SERVER['HTTP_X_ugmart_SIGNATURE'] !== hash_hmac( 'sha512', $json, $secret_key ) ) {
			exit;
		}
